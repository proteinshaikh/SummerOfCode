...
public class Evaluate_Division {
    ...
    public static void main(String[] args) {
        Evaluate_Division evaluate_division = new Evaluate_Division();
        String[] strings = new String[]{"a", "b"};
        List<List<String>> equation = Arrays.asList(Arrays.asList(strings));
        String[] val = new String[]{"a", "b"};
        List<List<String>> vals = Arrays.asList(Arrays.asList(strings));
        double[] doubles = new double[]{0.5};
        List<double[]> list = new ArrayList<>();
        for (int i = 0; i < val.length / 2; i++) {
            list.add(Arrays.stream(evaluate_division.calcEquation(equation, doubles, vals)).toArray());
